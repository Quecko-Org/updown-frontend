...
function SideTable({
  ...
  const hasOrders = rows.length > 0 || bids.length > 0;
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-white shadow-card">
      <div
        className={cn(
          ...
          accent === "up" ? "bg-[rgba(20,158,97,0.16)]" : "bg-[rgba(225,29,72,0.16)]"
        )}
      >
        <p className="text-sm font-bold text-neutral-ink">{title}</p>
      </div>
      {!hasOrders ? (
        <div className="flex min-h-[140px] items-center justify-center px-4 py-10">
          <p className="text-center text-sm font-medium text-muted">No orders yet</p>
        </div>
      ) : (
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <SideTable title="UP" snapshot={data.up} accent="up" />
        <SideTable title="DOWN" snapshot={data.down} accent="down" />
      </div>
    </div>
  );
}
